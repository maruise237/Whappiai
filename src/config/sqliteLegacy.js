/**
 * Legacy SQLite compatibility layer
 *
 * The active Whappi runtime now uses Postgres through `src/db/query.js`.
 * This file exists only for older tests, migration utilities, and one-off
...
function initializeSchema() {
    ...
    runner.run('rename-animator-to-engagement-fix-v3', (db) => {
        ...
        if (tableCheck) {
            ...
            if (!newTableCheck) {
                log('Migration de donnÃ©es : Renommage de group_animator_tasks en group_engagement_tasks', 'SYSTEM');
    ...
    runner.run('whatsapp-sessions-repair-v2026-v8-final-v2', (db) => {
        ...
        if (!info.some(c => c.name === 'qr_code')) {
            try {
                ...
                log("Migration : Colonne qr_code ajoutÃ©e avec succÃ¨s", "SYSTEM");
            } catch (e) {
                log("Migration : Ã‰chec ajout qr_code", "SYSTEM", { error: e.message }, "ERROR");
            }
        }
    });
    ...
    runner.run('users-add-chariow-license-key-v1', (db) => {
        ...
        if (!info.some(c => c.name === 'chariow_license_key')) {
            try {
                ...
                log("Migration : Colonne chariow_license_key ajoutÃ©e avec succÃ¨s", "SYSTEM");
            } catch (e) {
                log("Migration : Ã‰chec ajout chariow_license_key", "SYSTEM", { error: e.message }, "ERROR");
            }
        }
    });
    ...
    runner.run('seed-pricing-plans-v1', (db) => {
        ...
        if (count === 0) {
            ...
            log("Migration : Plans de tarification initialisÃ©s avec succÃ¨s", "SYSTEM");
        }
    });
        ...
        columns.forEach(col => {
            if (!info.some(c => c.name === col.name)) {
                try {
                    ...
                    log(`Migration : Colonne ${col.name} ajoutÃ©e avec succÃ¨s`, "SYSTEM");
                } catch (e) {
                    log(`Migration : Ã‰chec ajout ${col.name}`, "SYSTEM", { error: e.message }, "ERROR");
                }
            }
        });
    });
    ...
    log('SchÃ©ma SQLite legacy initialisÃ© avec succÃ¨s', 'SYSTEM', { event: 'db-schema-init-legacy' }, 'INFO');
    ...
    db.exec(`
        CREATE TABLE IF NOT EXISTS maintenance_settings (
            ...
            message TEXT DEFAULT 'Nous effectuons des amÃ©liorations techniques. Revenez dans quelques instants.',
    ...
    if (!maintenanceExists) {
        db.exec(`
            ...
            VALUES (1, 0, 'Maintenance en cours', 'Nous effectuons des amÃ©liorations techniques. Revenez dans quelques instants.', 'Wrench')
        `);
    }
}

function addPerformanceIndexes() {
    log('Ajout des index de performance legacy SQLite...', 'SYSTEM');
    ...
    log('Index de performance legacy SQLite ajoutÃ©s avec succÃ¨s', 'SYSTEM', { event: 'db-indexes-added-legacy' }, 'INFO');
}
