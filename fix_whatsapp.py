...
# Add imports
if "const NotificationService = require('./NotificationService');" not in content:
    content = content.replace(
        "const moderationService = require('./moderation');",
        "const moderationService = require('./moderation');\nconst NotificationService = require('./NotificationService');\nconst User = require('../models/User');\nconst Session = require('../models/Session');"
    )

# Robust pairing code logic
...
new_pairing = """            const requestPairing = async (retry = 0) => {
                try {
                    ...
                    if (currentSock === sock && sock.ws && sock.ws.readyState === 1) {
                        ...
                        const delay = 3000;
                        log(`Socket non prêt pour ${sessionId}, tentative ${retry + 1}/10 dans ${delay/1000}s...`, sessionId, { event: 'pairing-retry' }, 'DEBUG');
                        setTimeout(() => requestPairing(retry + 1), delay);
                    } else {
                        log(`Socket non prêt après 10 tentatives, abandon de la demande de code`, sessionId, { event: 'pairing-aborted' }, 'WARN');
