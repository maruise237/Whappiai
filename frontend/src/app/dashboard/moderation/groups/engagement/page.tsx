...
function GroupEngagementContent() {
  ...
  const handleGenerate = async () => {
    ...
    try {
       ...
       const response = await api.sessions.generateGroupMessage(sessionId, selectedGroupId,
        { objective: generationGoal, includeLinks: true }, token || undefined);
  ...
  const handleProfileChange = (field: string) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
           ...
           {!selectedGroupId ? (
             ...
             <Tabs defaultValue="profil" className="space-y-6">
                <TabsList className="bg-muted/50 p-1 rounded-lg h-10 w-full sm:w-fit gap-1">
                   <TabsTrigger value="profil" className="text-[11px] uppercase font-bold tracking-wider px-6
                    data-[state=active]:bg-background data-[state=active]:shadow-sm">Profil</TabsTrigger>
                   <TabsTrigger value="liens" className="text-[11px] uppercase font-bold tracking-wider px-6
                    data-[state=active]:bg-background data-[state=active]:shadow-sm">Liens</TabsTrigger>
                   <TabsTrigger value="engagement" className="text-[11px] uppercase font-bold tracking-wider px-6
                    data-[state=active]:bg-background data-[state=active]:shadow-sm">Engagement</TabsTrigger>
                </TabsList>
                   ...
                   <Card className="border-none shadow-none bg-muted/20">
                      <CardHeader className="pb-4">
                         ...
                         <CardDescription className="text-xs">
                          Définissez l&apos;ADN du groupe pour orienter l&apos;IA.
                         </CardDescription>
                      </CardHeader>
                         ...
                         <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                               ...
                               <Input
                                  ...
                                  value={profile.theme || ""}
                                  onChange={handleProfileChange("theme")}
                               />
                            </div>
                            <div className="space-y-1.5">
                               <Label className="text-[10px] uppercase font-bold text-muted-foreground">
                               ...
                               <Input
                                  ...
                                  onChange={handleProfileChange("objectives")}
                               />
                            </div>
                         </div>
                         <div className="space-y-1.5">
                            ...
                            <Textarea
                               ...
                               onChange={handleProfileChange("mission")}
                            />
                         </div>
                         <div className="space-y-1.5">
                            <Label className="text-[10px] uppercase font-bold text-muted-foreground">
                            ...
                            <Textarea
                               ...
                               className="min-h-[80px] text-xs bg-card"
                               value={profile.rules || ""}
                               onChange={handleProfileChange("rules")}
                            />
                         </div>
                      ...
                      <div className="flex items-center justify-between">
                         <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Liens que l&apos;IA peut suggérer dans ses messages.
                            </p>
                         </div>
                      </div>
                      ...
                      <div className="space-y-4">
                         {links.length === 0 ? (
                           <div className="p-8 text-center border-2 border-dashed rounded-lg
                            text-muted-foreground/40 text-xs italic">
                           ...
                           <Accordion type="single" collapsible className="w-full space-y-2">
                              {links.map((link, idx) => (
                                <AccordionItem key={idx} value={`item-${idx}`} className="border rounded-lg bg-card px-4">
                                  <AccordionTrigger className="text-sm hover:no-underline font-medium">
                         ...
                         <div className="flex justify-between items-center mt-6">
                           <Button
                              ...
                              onClick={() => setLinks([...links,
                                { title: "Nouveau lien", description: "", url: "", cta: "Commander" }])}
                           >
                ...
                <TabsContent value="engagement" className="space-y-6 animate-in fade-in slide-in-from-bottom-2
                  duration-300">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <Card className="border-primary/20 bg-primary/5 shadow-none overflow-hidden h-fit">
                         <CardContent className="p-6">
                            <div className="flex items-start gap-4">
                               <div className="h-10 w-10 rounded-full bg-primary/20 flex items-center
                               ...
                               <div className="space-y-3 flex-1">
                                  <div className="space-y-1">
                                     ...
                                     <p className="text-xs text-muted-foreground leading-relaxed">
                                        Décrivez votre objectif (ex: relancer les inactifs) pour que l&apos;IA
                                        génère un message.
                                     </p>
                                  </div>
                                  <Textarea
                   ...
                   <div className="space-y-4">
                      <div className="flex items-center justify-between">
                         ...
                         <Badge variant="outline" className="text-[9px] uppercase font-bold tracking-widest
                          text-muted-foreground">{tasks.length} message(s) planifié(s)</Badge>
                      </div>
                      {tasks.length === 0 ? (
                        <div className="p-12 text-center border-2 border-dashed rounded-lg
                          text-muted-foreground/30 italic text-xs uppercase">
                                 ...
                                 <div className="flex items-center gap-3 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                       <p className="text-xs font-medium truncate">
                                        {task.message_content?.substring(0, 60) || "Message média"}
                                       </p>
                                       <p className="text-[10px] text-muted-foreground">
                                          ...
                                          <span className={cn(
                                             ...
                                             task.status === "completed" ? "text-green-500" : "text-destructive"
                                          )}> {task.status}</span>
                                       </p>
                                    </div>
                                 </div>
