...
function handleMessageTypeChange() {
    ...
    // Show/hide media upload section
    document.getElementById('mediaUploadSection').style.display =
        messageType !== 'text' ? 'block' : 'none';
        
    // Show/hide audio options (PTT)
    document.getElementById('audioOptions').style.display =
        messageType === 'audio' ? 'block' : 'none';
        
    // Show/hide caption section (Message Content)
    // In WhatsApp, captions are supported for images and videos. 
    // Documents also support captions in some cases. 
    // Audio does not support captions.
    const captionContainer = document.querySelector('#messageEditor').closest('.mb-3');
    if (captionContainer) {
        captionContainer.style.display = messageType === 'audio' ? 'none' : 'block';
    }
}
...
async function createCampaign() {
    const campaignData = {
        ...
        message: {
            ...
            ptt: document.getElementById('pttToggle').checked,
            mediaCaption: (document.getElementById('messageType').value === 'image' || document.getElementById('messageType').value === 'video' || document.getElementById('messageType').value === 'document') ? quillEditor.root.innerHTML : null
        },
    ...
    try {
        ...
        if (!campaignData.scheduledAt) {
            const result = await Swal.fire({
                ...
                text: 'Do you want to start sending now?',
                icon: 'question',
...
async function sendCampaign(campaignId) {
    const result = await Swal.fire({
        ...
        text: 'Are you sure you want to start sending this campaign?',
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#10b981',
        cancelButtonColor: '#6b7280',
...
async function retryCampaign(campaignId) {
    const result = await Swal.fire({
        title: 'Retry Failed?',
        text: 'Retry sending to all failed recipients?',
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#10b981',
        cancelButtonColor: '#6b7280',
...
function showAlert(message, type = 'info') {
    ...
    return Swal.fire({
        ...
        showConfirmButton: !(type === 'success' || type === 'info')
    });
}
...
function debugCSVUpload() {
    ...
    Swal.fire({
        title: 'Debug Info',
        text: 'Debug info logged to console. Press F12 to view.',
        icon: 'info',
        confirmButtonColor: '#10b981'
    });
}
...
async function removeRecipientFromEditList(number) {
    ...
    const result = await Swal.fire({
        ...
        text: 'Remove this recipient from the list?',
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#ef4444',
        cancelButtonColor: '#6b7280',
...
async function deleteRecipientList(listId) {
    const result = await Swal.fire({
        ...
        text: 'Are you sure you want to delete this recipient list? This action cannot be undone.',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#ef4444',
        cancelButtonColor: '#6b7280',
...
async function sendCampaignFromDetail(campaignId) {
    const result = await Swal.fire({
        title: 'Start Campaign?',
        text: 'Are you sure you want to start sending this campaign? This will send messages to all recipients in the list.',
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#10b981',
        cancelButtonColor: '#6b7280',
...
async function checkOverdueCampaigns() {
    try {
        ...
        if (data.overdueCampaigns > 0) {
            ...
            const result = await Swal.fire({
                title: 'Overdue Campaigns Found',
                html: `Found ${data.overdueCampaigns} overdue campaign(s):<br><br>${campaignsList}<br><br>Would you like to manually start them now?`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#10b981',
                cancelButtonColor: '#6b7280',
